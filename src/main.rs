...
fn main() -> io::Result<()> {
    ...
    let _nom_img = match store(&args[1], &args[2], &args[3]) {
        Ok(a) => a,
        Err(err) => {
            eprintln!("error: {}", err);
            std::process::exit(1);
    ...
    let _a = load(&args[3], "pruebaaaa.txt");
...
#[allow(dead_code)]
fn store(img_path: &str, file_path: &str, output_file: &str) -> io::Result<String> {
...
#[allow(dead_code)]
fn load(img_path: &str, file_path: &str) -> io::Result<()> {
    ...
    for it in iterator {
        ...
        byte.push_str(&restr);
        if it.0 % 3 == 2 {
            let a = byte.pop(); // el ultimo elemento no es parte del byte
            let ret: u32 = bin_to_byte(&byte);
            let byte_value = char::from_u32(ret).unwrap();
